...
const errorStore = {
   ...
   * @param {Object} additionalData - Any additional relevant data
   */
  function logError(error, context = 'unknown', isFatal = false, additionalData = {}) {
    // Create structured error record
    const errorRecord = errorStore.addError(error, context, isFatal, additionalData);
    ...
    logMethod(`[${timestamp}] ${isFatal ? 'FATAL ERROR' : 'ERROR'} in ${context}: ${error.message}`);
    ...
    if (typeof window !== 'undefined' && window.location.hostname.includes('ptcg-sim-meta.pages.dev')) {
  ...
  function shouldSendToMonitoring(errorRecord) {
    ...
    if (errorRecord.type === 'NetworkError' && 
  ...
  function createUserFriendlyMessage(error, context) {
    ...
    const errorTypeMap = {
      SyntaxError: 'There was a problem with the code. Please try refreshing the page.',
      ReferenceError: 'There was a programming error. Please try refreshing the page.',
      TypeError: 'There was a type mismatch in the code. Please try refreshing the page.',
      NetworkError: 'There was a network issue. Please check your internet connection and try again.',
      TimeoutError: 'The operation timed out. Please try again.',
      'AbortError': 'The operation was cancelled.',
      'QuotaExceededError': 'The storage quota has been exceeded. Please clear some space and try again.'
    };
    
    // Context-specific messages
    const contextMap = {
      'socket': 'There was an issue with the connection. Please try refreshing the page.',
      ...
      'storage': 'There was an issue saving your data. Please make sure you have enough storage space.',
      'import': 'There was an issue importing your data. Please check the format and try again.',
      'export': 'There was an issue exporting your data. Please try again.'
    };
    
    // Try to get a message based on error type
    const typeMessage = errorTypeMap[error.name] || 'An unexpected error occurred. Please try again.';
   ...
   */
  function displayErrorToUser(message, level = 'error', duration = 5000, onAction = null) {
    ...
    errorElement.style.backgroundColor = level === 'error' ? '#f44336' : 
                                        level === 'warning' ? '#ff9800' : '#2196f3';
   ...
   */
  function withErrorHandling(fn, context = 'unknown', isFatal = false, onError = null) {
   ...
   */
  function withAsyncErrorHandling(fn, context = 'unknown', isFatal = false, onError = null) {

...
export class RaidClientCore {
  ...
  connect() {
    ...
    if (typeof io === 'undefined') {
      this.log(
        'RaidClientCore: Socket.IO client (io) not found!',
        'error',
        true
      );
      raidErrorHandler.logError(
        raidErrorHandler.codes.SYSTEM,
        'Socket.IO client not loaded.'
      );
      this.isAttemptingConnection = false;
      this.uiController.updateConnectionStatus(
        'Error: Socket.IO missing',
        'error'
      );
      return;
    }
  ...
  setupSocketListeners() {
    ...
    this.socket.on('connect_error', (error) => {
      this.isAttemptingConnection = false;
      this.log(
        `RaidClientCore: Connection error: ${error.message}`,
        'error',
        true
      );
      raidErrorHandler.logError(
        raidErrorHandler.codes.CONNECTION_LOST,
    ...
    this.socket.on('playerJoinedRaid', (data) =>
      this.handlePlayerJoinedRaid(data)
    );
    ...
    this.socket.on('raidActionResult', (data) =>
      this.handleRaidActionResult(data)
    );
    this.socket.on('raidActionFailed', (data) =>
      this.handleRaidActionFailed(data)
    );
  ...
  log(message, type = 'info', isCritical = false) {
    ...
    if (this.uiController) {
      ...
      if (this.uiController.config.showDebugInfo) {
        this.uiController.updateDebugInfo(
          {
            username: this.username,
            layoutPreference: this._layoutPreference,
  ...
  createRaid(preferredLayout = 'versus', customRaidId = null) {
    console.log(
      `RaidClientCore: createRaid(Layout: ${preferredLayout}, CustomID: ${customRaidId}) called.`
    );
    if (!this.socket || !this.socket.connected) {
    ...
    const raidIdToCreate =
      customRaidId ||
      `${this.sessionManager.config.sessionPrefix}-${Date.now().toString(36).slice(-4)}-${Math.random().toString(36).substring(2, 6)}`.toUpperCase();

    this.log(
      `Requesting create raid: ${raidIdToCreate} (Layout: ${this._layoutPreference})`
    );
  ...
  joinRaid(raidId, username) {
    console.log(
      `RaidClientCore: joinRaid(RaidID: ${raidId}, User: ${username}) called.`
    );
    if (!this.socket || !this.socket.connected) {
    ...
    this.uiController.logStatus(
      `Attempting to join raid: ${raidId} as ${this.username}`
    );
  ...
  sendRaidAction(actionType, payload) {
    console.log(
      `RaidClientCore: sendRaidAction(Type: ${actionType}, Payload: ${JSON.stringify(payload)}) called.`
    );
    if (!this.socket || !this.socket.connected || !this.currentRaidId) {
      this.log(
        `Cannot send '${actionType}': Not in a raid or not connected.`,
        'error',
        true
      );
      this.uiController.showNotification(
        'Not in a raid or disconnected!',
        'error'
      );
      return;
    }
  ...
  requestLayoutSwitch(newLayout) {
    console.log(
      `RaidClientCore: requestLayoutSwitch(NewLayout: ${newLayout}) called.`
    );
    if (!this.socket || !this.socket.connected || !this.currentRaidId) {
      this.log(
        'Cannot switch layout: Not in a raid or not connected.',
        'warning'
      );
      return;
    }
    if (this.raidState && this.raidState.layout === newLayout) {
      ...
      this.uiController.showNotification(
        `Layout is already ${newLayout}.`,
        'info'
      );
      return;
    }
    this.log(`Requesting layout switch to: ${newLayout}`);
    this.socket.emit('switchLayout', {
      raidId: this.currentRaidId,
      layout: newLayout,
    });
  }
  ...
  handleRaidCreated(data) {
    ...
    if (data.success) {
      this.log(
        `Raid created: ${data.raidId}. You are: ${data.playerId}`,
        'success'
      );
      ...
      this.username =
        this.raidState.players.find((p) => p.id === this.playerId)?.username ||
        this.username;

      if (this.uiController) {
        this.uiController.updateSessionDisplay(this.raidState);
        this.uiController.showNotification(
          `Raid ${data.raidId} created! Share ID.`,
          'success',
          5000
        );
      ...
      if (this.uiController)
        this.uiController.showNotification(
          `Raid creation failed: ${data.error}`,
          'error'
        );
    }
  }
  ...
  handleRaidJoined(data) {
    ...
    if (data.success) {
      this.log(
        `Joined raid: ${data.raidState.id}. Your ID: ${data.playerId}`,
        'success'
      );
      ...
      if (this.uiController) {
        this.uiController.updateSessionDisplay(this.raidState);
        this.uiController.showNotification(
          `Joined raid: ${this.currentRaidId}`,
          'success'
        );
        this.uiController.switchView('game');
        this.uiController.updateRaidFullDisplay(this.raidState, this.playerId);
      ...
      if (this.uiController)
        this.uiController.showNotification(
          `Failed to join raid: ${data.error}`,
          'error'
        );
      this.currentRaidId = null;
    }
  ...
  handleRaidJoinFailed(data) {
    ...
    this.log(
      `Raid join failed: ${data.message || 'Unknown'} (Code: ${data.code || 'N/A'})`,
      'error',
      true
    );
    if (this.uiController)
      this.uiController.showNotification(
        `Join failed: ${data.message}`,
        'error'
      );
  }
  ...
  handlePlayerJoinedRaid(data) {
    ...
    this.log(
      `Player ${data.player.username} joined. Total: ${data.playerCount}`,
      'info'
    );
  ...
  handlePlayerLeftRaid(data) {
    ...
    this.log(
      `Player ${data.playerUsername} left. Total: ${data.playerCount}`,
      'info'
    );
    if (this.currentRaidId === data.raidId) {
      if (data.playerId === this.playerId) {
        ...
        this.raidState = data.updatedRaidState;
        if (this.uiController) {
          this.uiController.updateRaidFullDisplay(
            this.raidState,
            this.playerId
          );
          this.uiController.updateSessionDisplay(this.raidState);
          this.uiController.showNotification(
            `${data.playerUsername} left.`,
            'info'
          );
        }
      }
  ...
  handleLayoutUpdated(data) {
    ...
    this.log(
      `Layout updated to ${data.layout} for raid ${data.raidId}.`,
      'info'
    );
    if (this.currentRaidId === data.raidId && data.updatedRaidState) {
  ...
  handleGameStateUpdate(data) {
    console.log(
      'RaidClientCore: handleGameStateUpdate received for phase:',
      data.raidState?.gamePhase
    );
    if (this.currentRaidId === data.raidId) {
      this.raidState = data.raidState;
      if (this.uiController) {
  ...
  handleRaidActionResult(data) {
    ...
    this.log(
      `Action '${data.actionType}' result: ${data.message}`,
  ...
  handleRaidActionFailed(data) {
    ...
    this.log(
      `Action failed: ${data.error} (Action: ${data.actionType})`,
      'error',
      true
    );
    if (this.uiController)
  ...
  handleRaidEnded(data) {
    ...
    this.log(
      `Raid ${data.raidId} ended. Reason: ${data.reason}. Victory: ${data.victory}`,
      data.victory ? 'success' : 'info'
    );
    if (this.uiController)
      this.uiController.showNotification(
        `Raid Ended: ${data.reason}`,
        data.victory ? 'success' : 'info',
        7000
      );
    if (this.currentRaidId === data.raidId) {
  ...
  handleRaidError(data) {
    ...
    this.log(
      `Server raid error for ${data.raidId || 'N/A'}: ${data.message} (Code: ${data.code || 'N/A'})`,
      'error',
      true
    );
    if (this.uiController)
      this.uiController.showNotification(`Error: ${data.message}`, 'error');
  }

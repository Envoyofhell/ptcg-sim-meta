...
export class RaidUIController {
  ...
  initializeBaseUI() {
    ...
    if (turnIndicatorContainer) {
      ...
      console.warn(
        '[RaidUIController] Turn indicator container (turnIndicatorContainer) not found.'
      );
    }
  ...
  updateRaidFullDisplay(raidState, currentPlayerId) {
    console.log(
      'RaidUIController: updateRaidFullDisplay called with raidState:',
      raidState,
      'currentPlayerId:',
      currentPlayerId
    );
    ...
    if (this.turnIndicatorBar && raidState.turnIndicator) {
      this.turnIndicatorBar.handleTurnUpdate({
        turnIndicator: raidState.turnIndicator,
      });
    ...
    if (this.elements.turnIndicatorContainer) {
      this.elements.turnIndicatorContainer.style.display =
        raidState.gamePhase === 'playing' || raidState.gamePhase === 'bossTurn'
          ? 'block'
          : 'none';
    }
    ...
    if (this.config.showDebugInfo && this.clientCore) {
      this.updateDebugInfo(
        {
          username: this.clientCore.username,
          layoutPreference: this.clientCore.getLayoutPreference(),
          currentRaidId: this.clientCore.currentRaidId,
        },
        raidState
      );
    }
  ...
  updatePlayerVisuals(playersData, layout, currentPlayerId, positions) {
    ...
    if (!container)
      return console.warn('raidPlayersContainer not found for visuals.');
    container.innerHTML = '';
    ...
    playersData.forEach((player) => {
      ...
      playerEl.style.background =
        player.id === currentPlayerId
          ...
          : colors[playerIndex % colors.length] || '#7f8c8d';
  ...
  updateBossVisual(bossData, positions) {
    ...
    if (bossHPEl)
      bossHPEl.textContent = `HP: ${bossData.currentHP || 0} / ${bossData.maxHP || 0}`;
  ...
  updateControlPanel(raidState, currentPlayerId) {
    if (!this.elements.raidControls)
      return console.warn('raidControls element not found.');

    const playerPosition = raidState.positions?.players?.find(
      (p) => p.playerId === currentPlayerId
    );

    if (this.elements.controlRaidId)
      this.elements.controlRaidId.textContent = raidState.id || '-';
    if (this.elements.controlPlayerCount)
      this.elements.controlPlayerCount.textContent = `${raidState.players?.length || 0}/${raidState.maxPlayers || 4}`;
    if (this.elements.controlLayoutDisplay)
      this.elements.controlLayoutDisplay.textContent = raidState.layout || '-';
    if (this.elements.controlYourAngle)
      this.elements.controlYourAngle.textContent = playerPosition
        ? Math.round(playerPosition.angle)
        : '-';
    if (this.elements.controlRaidPhase)
      this.elements.controlRaidPhase.textContent =
        raidState.gamePhase || 'Lobby';
  }

  updateLayoutIndicatorGfx(layout) {
    if (!layout) return;
    if (this.elements.layoutNameDisplay)
      this.elements.layoutNameDisplay.textContent =
  ...
  updateActionButtons(raidState, currentPlayerId) {
    if (!raidState) return;
    const isMyTurn =
      raidState.gamePhase === 'playing' &&
      raidState.currentTurnPlayerId === currentPlayerId;
    const gameIsActive =
      raidState.gamePhase === 'playing' || raidState.gamePhase === 'bossTurn';
  ...
  switchView(viewName) {
    ...
    if (viewName === 'lobby') {
      ...
      else console.warn('raidLauncher not found for lobby view');
      ...
      else console.warn('raidContainer not found for game view');
      if (controls) controls.style.display = 'block';
      else console.warn('raidControls not found for game view');
      if (actions) actions.style.display = 'flex';
      else console.warn('gameActions not found for game view');
  ...
  createSessionDisplay() {
    ...
    if (!launcher)
      return console.warn(
        'raidLauncher not found for session display creation.'
      );
    ...
    this.elements.currentSessionIdInput = document.getElementById(
      'currentSessionIdInput'
    );
    this.elements.copySessionIdBtn =
      document.getElementById('copySessionIdBtn');
    this.elements.sessionPlayerCountDisplay = document.getElementById(
      'sessionPlayerCountDisplay'
    );
    this.elements.sessionStatusDisplay = document.getElementById(
      'sessionStatusDisplay'
    );
    this.elements.refreshSessionBtn =
      document.getElementById('refreshSessionBtn');

    if (this.elements.copySessionIdBtn) {
      this.elements.copySessionIdBtn.addEventListener('click', () =>
  ...
  updateSessionDisplay(raidState) {
    console.log(
      'RaidUIController: updateSessionDisplay called with raidState:',
      raidState
    );
    if (!raidState) {
    ...
    if (this.elements.currentSessionIdInput)
      this.elements.currentSessionIdInput.value = raidState.id;
    if (this.elements.sessionPlayerCountDisplay)
      this.elements.sessionPlayerCountDisplay.textContent = `${raidState.players?.length || 0}/${raidState.maxPlayers || 4}`;
    if (this.elements.sessionStatusDisplay) {
      this.elements.sessionStatusDisplay.textContent =
        raidState.gamePhase || 'Lobby';
  ...
  clearSessionDisplay() {
    ...
    if (this.elements.currentSessionIdInput)
      this.elements.currentSessionIdInput.value = '';
    if (this.elements.sessionPlayerCountDisplay)
      this.elements.sessionPlayerCountDisplay.textContent = '-';
    if (this.elements.sessionStatusDisplay) {
  ...
  copySessionIdToClipboard() {
    ...
    const sessionId = this.elements.currentSessionIdInput
      ? this.elements.currentSessionIdInput.value
      : null;
    if (!sessionId) {
  ...
  setRaidIdInputValue(raidId) {
    ...
    if (
      this.elements.currentSessionIdInput &&
      (!this.elements.currentSessionIdInput.value ||
  ...
  showNotification(message, type = 'info', duration = 4000) {
    console.log(
      `RaidUIController: showNotification (Type: ${type}, Msg: ${message})`
    );
  ...
  createDebugPanel() {
    ...
    this.elements.debugClientCoreState = document.getElementById(
      'debugClientCoreState'
    );
    this.elements.debugUiControllerState = document.getElementById(
      'debugUiControllerState'
    );
    this.elements.debugLastRaidState =
      document.getElementById('debugLastRaidState');
    this.elements.debugRecentErrors =
      document.getElementById('debugRecentErrors');
  ...
  toggleDebug(show) {
    this.config.showDebugInfo =
      show === undefined ? !this.config.showDebugInfo : show;
    console.log(
      `RaidUIController: toggleDebug(${this.config.showDebugInfo}) called.`
    );
    if (this.elements.debugPanel) {
      this.elements.debugPanel.style.display = this.config.showDebugInfo
        ? 'block'
        : 'none';
    }
    if (this.config.showDebugInfo && this.clientCore) {
      // Ensure clientCore is available
      this.updateDebugInfo(
        {
          username: this.clientCore.username,
          layoutPreference: this.clientCore.getLayoutPreference(),
  ...
  updateDebugInfo(clientCoreSnapshot = {}, currentRaidStateSnapshot = {}) {
    ...
    if (this.elements.debugClientCoreState) {
      this.elements.debugClientCoreState.textContent = JSON.stringify(
        clientCoreSnapshot,
        null,
        2
      );
    ...
    if (this.elements.debugUiControllerState) {
      ...
      this.elements.debugUiControllerState.textContent = JSON.stringify(
        uiStateSnapshot,
        null,
        2
      );
    ...
    if (this.elements.debugLastRaidState) {
      this.elements.debugLastRaidState.textContent = JSON.stringify(
        currentRaidStateSnapshot,
        null,
        2
      );
    }
    if (this.elements.debugRecentErrors) {
      ...
      this.elements.debugRecentErrors.textContent = JSON.stringify(
        errors,
        null,
        2
      );
  ...
  displayErrorToUser(error) {
    console.error(
      `RaidUIController: displayErrorToUser: ${error.message} (Code: ${error.code})`
    );
    ...
    this.logStatus(
      `ERROR: ${error.message} (Code: ${error.code}, Details: ${JSON.stringify(error.details || {})})`,
      'error'
    );
    if (this.config.showDebugInfo && this.clientCore) {
      // Ensure clientCore is available
  ...
  setCreatingRaid(isCreating, raidId = '') {
    console.log(
      `RaidUIController: setCreatingRaid(${isCreating}, RaidID: ${raidId})`
    );
    this.state.creatingRaid = isCreating;
    if (this.elements.createRaidBtn)
      this.elements.createRaidBtn.disabled = isCreating;
    if (this.elements.joinRaidBtn)
      this.elements.joinRaidBtn.disabled = isCreating;
    if (isCreating) {
      this.logStatus(
        `Creating raid ${raidId ? '(' + raidId + ') ' : ''}...`,
        'info'
      );
    }
  }

...
class SocketService {
  ...
  setupEventHandlers() {
    ...
    this.socket.on('disconnect', (reason) => {
      ...
      if (reason === 'io server disconnect') {
        ...
        log(
          'Server initiated disconnect, not attempting to reconnect',
          'warn',
          'socket'
        );
      }
    ...
    this.socket.on('connect_error', (error) => {
      ...
      if (++this.reconnectAttempts >= this.maxReconnectAttempts) {
        this.switchToOfflineMode(
          `Failed after ${this.maxReconnectAttempts} attempts`
        );
      }
    });
  ...
  handleOfflineStoreGameState(data) {
    try {
      ...
      const handlers =
        this.eventHandlers.get('exportGameStateSuccessful') || [];
      handlers.forEach((handler) => handler(key));

      log(`Game state saved locally with key: ${key}`, 'info', 'storage');
    } catch (error) {
      log(
        `Error saving game state locally: ${error.message}`,
        'error',
        'storage'
      );
  ...
  displayConnectionStatus(connected, reason = '') {
    ...
    if (statusElement) {
      ...
      newStatusElement.className = connected
        ? 'connection-status connected'
        : 'connection-status disconnected';

      newStatusElement.textContent = connected
        ? 'Connected'
        : `Disconnected: ${reason}`;
      ...
      newStatusElement.style.backgroundColor = connected
        ? '#2ecc71'
        : '#e74c3c';

...
export function initializeSystemState() {
  ...
  try {
    ...
    logger.log(
      'Local storage not available, some offline features may be limited',
      'warn'
    );
  }
...
export function createSocketConnection(systemState) {
  try {
    ...
    if (typeof io === 'undefined') {
      logger.log(
        'Socket.IO library not available, switching to offline mode',
        'error'
      );
    ...
    socket.on('connect', () => {
      ...
      if (chatbox) {
        chatbox.innerHTML +=
          '<div style="color: green; font-weight: bold">Connected to server</div>';
      }

      // Process any pending messages
      if (
        window.pendingSocketMessages &&
        Array.isArray(window.pendingSocketMessages)
      ) {
        logger.log(
          `Processing ${window.pendingSocketMessages.length} pending messages`,
          'info'
        );
    ...
    socket.on('connect_error', (error) => {
      ...
      logger.log(
        `Socket.IO connection error (attempt ${systemState.connectionAttempts}/${systemState.maxConnectionAttempts}): ${error.message}`,
        'error'
      );
      ...
      if (systemState.connectionAttempts >= systemState.maxConnectionAttempts) {
        socket.disconnect();
        systemState.enableOfflineMode(
          `Failed after ${systemState.maxConnectionAttempts} attempts`
        );
        return createOfflineModeSocket(systemState);
      }
    });
    ...
    logger.log(
      `Error creating Socket.IO connection: ${error.message}`,
      'error'
    );
...
function handleOfflineStoreGameState(gameState, eventHandlers) {
  try {
    ...
    handlers.forEach((handler) =>
      handler('Error storing game state: ' + error.message)
    );
  }
}
...
export function safeSocketEmit(eventName, ...args) {
  try {
    ...
    if (socket.connected) {
      ...
      logger.log(
        `Queuing ${eventName} event for when socket reconnects`,
        'warn'
      );

...
async function fetchWithRetry(url, options = {}) {
  ...
  for (let attempt = 0; attempt <= CONFIG.maxRetries; attempt++) {
    try {
      ...
      lastError = error;
      log(
        `Request failed (attempt ${attempt + 1}/${CONFIG.maxRetries + 1}): ${error.message}`,
        'warn'
      );
...
export function loadImportData() {
  ...
  if (!key) {
    log(
      'No import key provided. Loading default state or showing welcome screen.',
      'info'
    );
  ...
  fetchWithRetry(url)
    .then((response) => {
      ...
      if (contentType && contentType.includes('text/html')) {
        ...
        return response.text().then((html) => {
          ...
          throw new Error(
            'Server returned HTML instead of JSON. The API endpoint may be misconfigured.'
          );
        });
      }
      ...
      if (!response.ok) {
        return processErrorResponse(response).then((errorDetails) => {
          throw new Error(
            `Server error (${errorDetails.status}): ${errorDetails.message}`
          );
        });
      }
    ...
    .then((importData) => {
      ...
      if (!Array.isArray(importData)) {
        ...
        log(
          `Data preview: ${JSON.stringify(importData).substring(0, 100)}...`,
          'debug'
        );
      ...
      log(
        `Successfully loaded game state with ${importData.length} actions`,
        'success'
      );
      ...
      try {
        ...
        actions.forEach((data, index) => {
          try {
            ...
            errors++;
            log(
              `Error applying action #${index}: ${actionError.message}`,
              'error'
            );
          }
        });
        ...
        displayMessage(
          `Game state loaded successfully! Applied ${appliedCount}/${actions.length} actions.`,
          'success'
        );
        ...
        if (errors > 0) {
          displayMessage(
            `Warning: ${errors} action(s) failed to apply.`,
            'warning'
          );
        }
        ...
        displayMessage(
          `Error processing game state: ${processingError.message}`,
          'error'
        );
      }
    })
    .catch((error) => {
      ...
      if (error.name === 'AbortError') {
        ...
        errorMessage =
          'The server returned an HTML page instead of game data. This may indicate a server configuration issue.';
      } else {

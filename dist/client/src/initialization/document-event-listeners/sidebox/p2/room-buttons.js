...
export const initializeRoomButtons = () => {
  ...
  roomHeaderCopyButton.addEventListener('click', () => {
    navigator.clipboard
      .writeText(systemState.roomId || roomIdInput.value)
      .then(() => {
  ...
  joinRoomButton.addEventListener('click', () => {
    ...
    if (socket && socket.connected) {
      ...
      const chatbox =
        document.getElementById('p2Chatbox') ||
  ...
  leaveRoomButton.addEventListener('click', () => {
    if (
      ...
      try {
        ...
        document.getElementById('flipBoardButton').style.display =
          'inline-block';

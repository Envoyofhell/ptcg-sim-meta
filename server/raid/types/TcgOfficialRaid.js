...
export class TCGOfficialRaid {
  ...
  calculateTotalPlayerDamage(players) {
    ...
    players.forEach((player) => {
      ...
      const pokemon1Damage = Math.max(
        ...player.pokemon[0].attacks.map((a) => a.damage)
      );
      const pokemon2Damage = Math.max(
        ...player.pokemon[1].attacks.map((a) => a.damage)
      );
  ...
  processCheerCard(gameState, player, action) {
    ...
    const effect = this.applyCheerCardEffect(
      action.cheerCardNumber,
      gameState,
      player
    );
  ...
  checkLossCondition(gameState) {
    return {
      ...
      reason:
        gameState.koCounters >= gameState.maxKOCounters
          ? "Too many Pokemon KO'd"
          : null,
    };
  }

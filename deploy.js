...
function deployToPages() {
  ...
  try {
    runCommand('pnpm wrangler --version', __dirname, true);
  } catch (error) {
    log('Wrangler not found. Installing...', 'warn');
    runCommand('pnpm add -g wrangler');
  }
  ...
  try {
    ...
    try {
      runCommand(
        `pnpm wrangler pages project get ptcg-sim-meta-${config.environment}`,
        __dirname,
        true
      );
    } catch (error) {
      ...
      runCommand(
        `pnpm wrangler pages project create ptcg-sim-meta-${config.environment} --production-branch main`
      );
    }

    // Deploy the client directory to Pages
    runCommand(
      `pnpm wrangler pages deploy ${path.join(config.distDir, 'client')} --project-name=ptcg-sim-meta-${config.environment} --commit-message="Deploy ${config.buildTimestamp}" --branch=${config.environment === 'production' ? 'main' : 'dev'}`
    );
...
function deployToWorkers() {
  ...
  try {
    ...
    // Deploy the worker
    runCommand(`pnpm wrangler deploy --env ${config.environment}`);
